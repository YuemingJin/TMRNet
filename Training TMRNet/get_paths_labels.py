...
mean_time = 38.28413888888889
std_time = 15.624871920042958
...
for j in range(len(phase_file_names)):
    ...
    for tool_line in tool_file:
        ...
        if int(tool_split[0]) % downsample_rate == 0:
            # time - minute
            time_el = float(tool_split[0])/float(downsample_rate)/60.0
            # time_el = (time_el - mean_time)/std_time
            time_el = time_el/mean_time

            for l in range(1, 8):
                info_all[count_tool].append(int(tool_split[l])) # info_each一个视频的一个帧img_file + phase + tool

            info_all[count_tool].append(float(time_el))
            count_tool += 1 
...
# TODO
    ...
    for tool_line in tool_file:
        ...
        if int(tool_split[0]) % downsample_rate == 0:
            ...
            time_el = float(tool_split[0])/downsample_rate/60
            # time_el = (time_el - mean_time)/std_time
            time_el = time_el/mean_time
            info_all[count_tool].append(float(time_el)) 
            count_tool += 1 
